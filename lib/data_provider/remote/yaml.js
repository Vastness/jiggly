    ...
    .then(function(loginUser) {
      ...
      return interfaceHandler.getHandler(service.path, functionName, params)
       .then(function(javaParams) {
         return nzd[serviceName][functionName].apply(this, javaParams);
       }).then(function(data) {
         debug('getServiceByFunctionName result', serviceName, functionName, data)
         if (data.success) {
           return data.result;
         }
       }).catch(function(err) {
         debug('getServiceByFunctionName', functionName, err)
       })
    })
}
