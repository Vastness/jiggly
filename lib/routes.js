...
module.exports = function(app) {
    ...
    _.forEach(urlsData, function(val, url) {
      ...
      if (_.isFunction(val)) {
        router.get(url, function(req, res, next) {
          debug('start router local', req.path)
      ...
      _.forEach(val, function(routeFn, method) {
        router[method](url, function(req, res, next) {
          debug('start router local', req.path, method)
          var result = {};
  ...
  .then(function() {
    ...
    app.all('/*', function(req, res, next) {
      debug('try proxy', req.path)
      dataProvider.proxyHttp(req, res)
        .then(function(isInterface) {
          if (!isInterface) {
            next()
          }
        })
    ...
    app.get(/^([^\.]+)$/, function(req, res, next) {
      debug('start render view', req.path)
