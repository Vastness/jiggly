  ...
  _.forEach(urlsData, function(val, url) {
    ...
    if (_.isFunction(val)) {
      router.get(url, function(req, res, next) {
        // 由于可能存在数据重新加载，需要实时去获取
        var result = dataProvider.getUrlHandle(url)(req.query, res);
        res.status(200).send(forceToString(result));
      });
    }
    ...
    _.forEach(val, function(routeFn, method) {
      router[method](url, function(req, res, next) {
        ...
        if (_.isFunction(routeFn)) {
          result = forceToString(dataProvider.getUrlHandle(method)(req.query, res));
        } else if (_.isObject(routeFn)) {
          result = dataProvider.getUrlHandle(method);
        }
